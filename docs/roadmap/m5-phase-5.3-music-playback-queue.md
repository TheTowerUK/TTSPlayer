...
## Live NAS manual validation (2026-07-21)

### Resolved — stale catalogue (not an app regression)

The live NAS catalogue was stale and generated by scanner **v0.3.3** using **schema v2**. A full scan with scanner **v0.4.0** generated **schema v3**, indexing **42,283** audio items with `media_kind` and music metadata. Test track `01 Oh Yeah.mp3` (Singles > CD Singles) validates correctly.

The client requires a **v3 catalogue** for music browsing, queue seeding, and playback. Reload the app after rescan so `CatalogService` picks up the new `catalog.json`.

### Pending — Release build smoke test

Close any running TTSPlayer instance and launch the current Release build:

`D:\AppDev\TTSPlayer\client\ttsplayer\build\windows\x64\runner\Release\ttsplayer.exe`

| # | Check |
|---|---|
| 1 | Open **Music** → browse **Singles > CD Singles** |
| 2 | Tracks appear as **music entries**, not image files |
| 3 | Open **Oh Yeah** — playback begins |
| 4 | Queue contains surrounding album/folder tracks |
| 5 | **Next**, **previous**, pause, seek, and natural completion work |
| 6 | Album folder with `cover.jpg` or `folder.jpg` — artwork shown; image **not** playable |

**Observation:** `duration_seconds: null` on some MP3s at scan time does not block playback. If duration stays unknown in UI after MediaKit loads, log separately as metadata enrichment — not a 5.3 queue blocker.

Automated coverage: `PHASE_53_RUNTIME=1` → `test/phase_53_music_player_windows_runtime_test.dart` (WAV fixtures; libmpv required).

### Operational validation note (lessons learned)

During manual validation, the application initially appeared to render music folders incorrectly. Investigation confirmed the client was loading a legacy `catalog.json` generated by scanner **v0.3.3** (catalogue schema **v2**). After regenerating the catalogue with scanner **v0.4.0** (schema **v3**), **42,283** audio items were indexed and the music experience behaved as designed.

**Future release validation should verify the catalogue version before investigating application behaviour.**
